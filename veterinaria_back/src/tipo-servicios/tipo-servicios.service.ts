...
export class TipoServiciosService {
  constructor(@InjectRepository(TipoServicio) private tiposerviciosRepository: Repository<TipoServicio>) {}
  async create(createTipoServicioDto: CreateTipoServicioDto):Promise<TipoServicio> {
    if (createTipoServicioDto.nombre){
      const tipoServicioExistente = await this.tiposerviciosRepository.findOneBy({
 ...
 async findOne(id: number): Promise<TipoServicio> {
    ...
    if (!tipoServicio) throw new NotFoundException('Tipo de servicio no encontrado');
    return tipoServicio;
  }

  async update(id: number, updateTipoServicioDto: UpdateTipoServicioDto):Promise<TipoServicio> {
