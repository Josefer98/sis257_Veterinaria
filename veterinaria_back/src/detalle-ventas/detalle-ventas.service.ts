...
export class DetalleVentasService {
 ...
 async create(createDetalleVentaDto: CreateDetalleVentaDto):Promise<DetalleVenta> {
    const detalleventa= this.detalleventasRepository.create(createDetalleVentaDto);
  ...
  async findOne(id: number):Promise<DetalleVenta> {
    const detalleventa= await this.detalleventasRepository.findOne({where:{id}});
    if(!detalleventa) throw new NotFoundException('detallede  venta no encontrado');
